...
BOOL TCPSocketRead(SOCKET s, BYTE* pBuffer, DWORD dwBufferSize, DWORD* pdwReaded, HANDLE hStopEvent)
{
    ...
    while (1)
    {
        ...
        {
            L_ERROR(_T("WSARecv failed with error: %d\r\n"), err);
            bRet = FALSE;
            break;
        }
        ...
        if (dwRet == WAIT_OBJECT_0)
        {
            ...
            if (rc == FALSE)
            {
                L_ERROR(_T("WSARecv operation failed with error: %d\r\n"), WSAGetLastError());
                bRet = FALSE;
                break;
            }
            if (RecvBytes == 0)
...
BOOL TCPSocketWrite(SOCKET s, BYTE* pBuffer, DWORD dwBufferSize, DWORD* pdwWritten, HANDLE hStopEvent)
{
    ...
    {
        L_INFO(_T("create wsaevent fail \r\n"));
        return FALSE;
    }
    ...
    do
    {
        ...
        if ((rc == SOCKET_ERROR) &&
            ...
            L_ERROR(_T("WSASend failed with error: %d\r\n"), err);
            break;
        }
